...
theme_config = {
    "dark": [colour(0, 0, 0), colour(255, 255, 255)],
    "light": [colour(255, 255, 255), colour(0, 0, 0)],
}
theme = "dark"

average_before = get_temp()
counter = 9
log = []

LCD.fill(theme_config[theme][0])
...
while True:
    ...
    if counter == 9:
        ...
        average = sum(log) / len(log)

        delta = average - average_before
        if delta > 0:
            accent = colour(255, 0, 0)
        elif delta < 0:
            accent = colour(0, 0, 255)
        else:
            accent = colour(128, 128, 128)

        average_before = average

        LCD.fill(theme_config[theme][0])

        printstring("Temperature", 20, 20, 3, False, False, theme_config[theme][1])
        printstring("Current", 20, 60, 2, False, False, theme_config[theme][1])
        printstring(
            f"{round(temp, 2)} [C]", 20, 85, 3, False, False, theme_config[theme][1]
        )
        printstring("Average", 20, 120, 2, False, False, theme_config[theme][1])
        printstring(f"{round(average, 2)} [C]", 20, 145, 3, False, False, accent)
        printstring(
            f"{round(temp * 1.8 + 32, 2)} [F]",
            20,
            190,
            2,
            False,
            False,
            theme_config[theme][1],
        )
        printstring(
            f"{round(temp + 273, 2)} [K]",
            20,
            210,
            2,
            False,
